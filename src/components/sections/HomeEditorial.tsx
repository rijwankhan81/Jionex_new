...
const serviceDescriptionKeys = [
  "Software Development Des",
  "IT Infrastructure Services Des",
  "Cybersecurity Solutions Des",
  "Digital Transformation Des",
  "Managed IT Services Des",
  "Cloud Computing Des",
  "Data Analytics and Business Intelligence Des",
  "Emerging Technologies Des",
  "Digital Marketing Des",
];
...
function splitTitle(value: string, className: string) {
  return value.split(/\s+/).map((word, index) => (
    <span className={className} key={`${word}-${index}`}>
      <span data-title-word>{word}</span>
    </span>
  ));
}
...
export default function HomeEditorial({ locale, translations }: Props) {
  ...
  const data = useMemo(() => {
    const byLocale = <T,>(en: T, bn: T, ar: T, es: T) =>
      locale === "bn" ? bn : locale === "ar" ? ar : locale === "es" ? es : en;
    return {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<HTMLElement>("[data-title]").forEach((title) => {
        gsap.fromTo(
          title.querySelectorAll("[data-title-word]"),
  ...
  return (
    <main ref={rootRef} className={styles.page}>
      <Hero
        locale={locale}
        heading={t("homeBannerHeading")}
        description={t("homeBannerDes")}
        startLabel={t("Let's get started")}
        productsLabel={t("View Products")}
      />
      ...
      <section className={`${styles.manifesto} ${styles.section}`} id="about">
        ...
        <div className={styles.manifestoGrid}>
          <div data-reveal>
            <p className={styles.eyebrow}>JIONEX / GLOBAL IT</p>
            <h2 data-title>{splitTitle(t("Why Jionex"), styles.word)}</h2>
          </div>
          <div className={styles.manifestoCopy} data-reveal>
            <p>{t("Our Portfolio Des")}</p>
            <Link href={path("about")} className={styles.textLink}>
              {t("About Us")} <FiArrowUpRight />
            </Link>
          </div>
        </div>
        <div className={styles.reasonGrid}>
          {[1, 2, 3, 4].map((item) => (
            <article key={item} className={styles.reason}>
              <span>0{item}</span>
              <p>{t(`Why C${item}`)}</p>
              <FiArrowUpRight />
            </article>
      ...
      <section
        className={`${styles.servicesSection} ${styles.section}`}
        id="services"
      >
        <div className={styles.sectionHead}>
          <div className={styles.rail} data-rail>
            ...
            <span>
              <i>{t("Services we offer")}</i>
            </span>
          </div>
          <h2>{splitTitle(t("Services we offer"), styles.word)}</h2>
        </div>
          ...
          {serviceKeys.map((key, index) => (
            ...
            >
              ...
              <div className={styles.serviceMain}>
                <h3>{t(key)}</h3>
                <p>{t(serviceDescriptionKeys[index])}</p>
              </div>
      ...
      <section className={`${styles.solutionsSection} ${styles.section}`}>
        <div className={styles.sectionHead}>
          ...
          <h2 data-title>{splitTitle(t("Our Products"), styles.word)}</h2>
        </div>
      ...
      <section
        className={`${styles.workSection} ${styles.section}`}
        id="portfolio"
      >
        <div className={styles.workHead}>
          ...
          <div>
            <p className={styles.eyebrow}>JIONEX / SELECTED WORK</p>
            <h2 data-title>{splitTitle(t("Our Portfolio"), styles.word)}</h2>
            <p>{t("Our Portfolio Des")}</p>
          </div>
        </div>
        ...
        <div className={styles.portfolioGrid}>
          {featuredProjects.map((project, index) => {
            ...
            return (
              <Link
                href={external ? project.slug : path("projects")}
                className={styles.portfolioMini}
                key={`${project.name}-${index + 3}`}
                data-reveal
                target={external ? "_blank" : undefined}
                rel={external ? "noreferrer" : undefined}
              >
                <div className={styles.portfolioMiniImage}>
                  <Image
                    src={project.image}
                    alt={project.name}
                    fill
                ...
                <div className={styles.portfolioMiniMeta}>
                  <span>0{index + 4}</span>
                  <h3>{project.name}</h3>
                  <FiArrowUpRight />
                </div>
        ...
        <Link href={path("projects")} className={styles.bigLink}>
          {t("See All")} <FiArrowUpRight />
        </Link>
      </section>

      <section
        className={`${styles.productsSection} ${styles.section}`}
        id="products"
      >
        <div className={styles.productsIntro}>
          ...
          <div>
            <p className={styles.eyebrow}>{t("Products Banner Sub Title")}</p>
            <h2 data-title>
              {splitTitle(t("Products Banner Title"), styles.word)}
            </h2>
            <p className={styles.lead}>{t("Products Banner Des")}</p>
          </div>
        </div>
        ...
        <Link href={path("products")} className={styles.bigLink}>
          {t("See All")} <FiArrowUpRight />
        </Link>
      </section>
      ...
      <section className={`${styles.industriesSection} ${styles.section}`}>
        <div className={styles.industryHead}>
          ...
          <div>
            <h2 data-title>
              {splitTitle(t("Industries We Covered"), styles.word)}
            </h2>
          </div>
        </div>
        <div className={styles.industryGroups}>
          {data.serviceGroups.map((group, index) => (
            <article
              key={group.category}
      ...
      <section
        className={`${styles.expertiseSection} ${styles.section}`}
        id="technology"
      >
        <div className={styles.rail} data-rail>
          <span>
          ...
          <span>
            <i>{t("Our Technology Stack")}</i>
          </span>
        </div>
        <div className={styles.expertiseGrid}>
          <div data-reveal>
            <h2 data-title>
              {splitTitle(t("Our Technology Stack"), styles.word)}
            </h2>
            <p className={styles.lead}>{t("Technologies We Use Des")}</p>
          </div>
          <div className={styles.techList}>
            {data.technologies.slice(0, 8).map((item, index) => (
              <Link
                href={path("technologies")}
                key={item.title}
      ...
      <section className={`${styles.abilitySection} ${styles.section}`}>
        <div className={styles.sectionHead}>
          ...
          <h2 data-title>{splitTitle(t("Exp"), styles.word)}</h2>
        </div>
        <div className={styles.abilityGrid}>
          {data.abilities.map((item, index) => (
            <article key={item.id} className={styles.abilityCard} data-reveal>
      ...
      <section className={`${styles.valuesSection} ${styles.section}`}>
        <div className={styles.valuesHead}>
          ...
          <div>
            <h2 data-title>{splitTitle(t("Values"), styles.word)}</h2>
            <p>{t("Values Des")}</p>
          </div>
        </div>
        <div className={styles.valuesList}>
          {data.values.map((value, index) => (
            <article className={styles.valueRow} key={value.title} data-reveal>
              <span>0{index + 1}</span>
              <h3>{value.title}</h3>
              <p>{value.description}</p>
              <Image src={value.image} alt="" width={90} height={90} />
            </article>
          ))}
        </div>
      </section>
      ...
      <section className={`${styles.clientsSection} ${styles.section}`}>
        <div className={styles.clientsHead}>
          ...
          <div>
            <h2 data-title>
              {splitTitle(t("Clients Banner Title"), styles.word)}
            </h2>
        ...
        <div className={styles.clientGrid}>
          {clients.map((client, index) => (
            <div className={styles.clientCard} key={client.id} data-reveal>
              <span>0{String(index + 1).padStart(2, "0")}</span>
              <div>{client.name}</div>
              <small>{client.country}</small>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className={`${styles.faqSection} ${styles.section}`}>
        ...
        <div className={styles.faqGrid}>
          <div data-reveal>
            <h2 data-title>{splitTitle(t("FAQ"), styles.word)}</h2>
            <p>{t("FAQ Des")}</p>
          </div>
          <div className={styles.faqList}>
            {faqItems.map((item, index) => (
              <button
                key={item}
                className={`${styles.faqItem} ${faqOpen === index ? styles.faqOpen : ""}`}
                onClick={() => setFaqOpen(faqOpen === index ? -1 : index)}
              >
                <span>0{index + 1}</span>
                <div>
                  <strong>{t(`FAQ H${item}`)}</strong>
                  {faqOpen === index && <p>{t(`FAQ H${item} Des`)}</p>}
                </div>
                <FiPlus />
              </button>
            ))}
      ...
      <section className={styles.finalCta} id="contact" data-reveal>
        ...
        <p>{t("Contact H Des")}</p>
        <Link href={path("contact")} className={styles.ctaButton}>
          {t("Request a Quote")} <FiArrowDownRight />
        </Link>
      </section>
    </main>
  );
